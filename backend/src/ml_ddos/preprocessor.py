...
def fit_invalid_value_medians(X_train: pd.DataFrame) -> pd.Series:
    """Fit numeric medians on the training split only."""
    numeric_cols = X_train.select_dtypes(include=[np.number]).columns
    train_numeric = X_train[numeric_cols].replace([np.inf, -np.inf], np.nan)
    return train_numeric.median().fillna(0)


def apply_invalid_value_medians(X: pd.DataFrame, medians: pd.Series,
                                name: str = "DataFrame") -> pd.DataFrame:
    """Replace invalid values and fill NaNs using train-fitted medians."""
    X = X.copy()
    numeric_cols = X.select_dtypes(include=[np.number]).columns

    n_inf = np.isinf(X[numeric_cols]).sum().sum()
    if n_inf > 0:
        print(f"[preprocessor] Replacing {n_inf:,} infinite values in {name}.")
        X[numeric_cols] = X[numeric_cols].replace([np.inf, -np.inf], np.nan)

    n_null = X[numeric_cols].isnull().sum().sum()
    if n_null > 0:
        print(f"[preprocessor] Filling {n_null:,} NaN values in {name} with train medians.")
...
def find_single_value_columns(X_train: pd.DataFrame) -> list[str]:
    """Find columns with a single unique value in the training split only."""
...
def find_highly_correlated_columns(X_train: pd.DataFrame,
                                   threshold: float = 0.8) -> list[str]:
    """Find correlated feature columns using the training split only."""
...
def preprocess(train_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    # Step 3: Train-validation split before fitting any feature transforms.
    # This keeps validation out of imputation, feature selection, and scaling.
    ...
    # Step 4: Handle invalid values with medians fitted on X_train only
    print("\n[Step 4] Handling invalid/infinite values ...")
    imputation_medians = fit_invalid_value_medians(X_train)
    X_train = apply_invalid_value_medians(X_train, imputation_medians, "training split")
    X_val = apply_invalid_value_medians(X_val, imputation_medians, "validation split")
    X_test = apply_invalid_value_medians(X_test, imputation_medians, "testing set")

    # Step 5: Drop single-value columns discovered on X_train only
    print("\n[Step 5] Dropping single-value columns ...")
    dropped_sv = find_single_value_columns(X_train)
    if dropped_sv:
        print(f"[preprocessor] Dropping {len(dropped_sv)} single-value columns: {dropped_sv}")
        X_train = X_train.drop(columns=dropped_sv)
        X_val = X_val.drop(columns=[c for c in dropped_sv if c in X_val.columns])
        X_test = X_test.drop(columns=[c for c in dropped_sv if c in X_test.columns])

    # Step 6: Drop highly correlated columns discovered on X_train only
    print("\n[Step 6] Dropping highly correlated columns ...")
    dropped_hc = find_highly_correlated_columns(X_train, threshold=correlation_threshold)
    if dropped_hc:
        print(f"[preprocessor] Dropping {len(dropped_hc)} highly correlated columns (>{correlation_threshold}).")
        X_train = X_train.drop(columns=dropped_hc)
        X_val = X_val.drop(columns=[c for c in dropped_hc if c in X_val.columns])
        X_test = X_test.drop(columns=[c for c in dropped_hc if c in X_test.columns])
    ...
    cleaned_train_df = pd.concat(
        [
            ...
            pd.concat([y_train, y_val], axis=0).rename(TARGET_COL),
        ],
        axis=1,
    )
    cleaned_test_df = pd.concat([X_test, y_test.rename(TARGET_COL)], axis=1)
    summary_after = generate_data_summary(cleaned_train_df, cleaned_test_df)
